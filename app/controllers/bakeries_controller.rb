...
class BakeriesController < ApplicationController
  ...
  def show
    ...
    if params[:search_recipe]
      ...
      @recipes = @bakery.recipes.paginate(:page => params[:page], :per_page => 10, :conditions => ['name like ?', "%#{q}%"]).order('name')
      ...
      @materials = @bakery.materials.paginate(:page => params[:page], :per_page => 5, :conditions => ['name like ?', "%#{q}%"]).order('name')
    else
      @recipes = @bakery.recipes.paginate(:page => params[:page], :per_page => 10).order('name')
      @materials = @bakery.materials.paginate(:page => params[:page], :per_page => 5).order('name')
    end
