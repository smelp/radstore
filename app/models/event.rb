...
class Event < ActiveRecord::Base
  ...
  def self.getRemovalsByDateRange (startTime, endTime)
    Event.where("\"event_type\" = 4  AND \"user_timestamp\" >='"+startTime.to_date.to_s+"' AND \"user_timestamp\" <='"+endTime.to_date.to_s+"'")
  end

  def self.getProductsByDateRange (startTime, endTime, product_event)
    Event.where("\"event_type\" = "+product_event.to_s+"  AND \"user_timestamp\" >='"+startTime.to_date.to_s+"' AND \"user_timestamp\" <='"+endTime.to_date.to_s+"'")
  end
